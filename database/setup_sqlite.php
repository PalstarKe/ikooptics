 ...
 * Run: php database/setup_sqlite.php
 */

$dbPath = __DIR__ . '/ikooptic.db';

if (file_exists($dbPath)) {
    echo "Database already exists. Delete it first to re-create.\n";
    echo "  rm database/ikooptic.db && php database/setup_sqlite.php\n";
    exit(0);
}
...
CREATE TABLE users (
    ...
    role TEXT DEFAULT 'support' CHECK(role IN ('admin','content_manager','support')),
    avatar VARCHAR(255),
...
CREATE TABLE coverage_areas (
    ...
    wireless_available INTEGER DEFAULT 0,
    latitude REAL,
    longitude REAL,
...
CREATE TABLE service_packages (
    ...
    description TEXT,
    category TEXT DEFAULT 'home' CHECK(category IN ('home','business','enterprise')),
...
CREATE TABLE client_requests (
    ...
    message TEXT,
    status TEXT DEFAULT 'new' CHECK(status IN ('new','contacted','installation_scheduled','completed','cancelled')),
...
CREATE TABLE settings (
    ...
    value TEXT,
    type TEXT DEFAULT 'text' CHECK(type IN ('text','textarea','html','image','json')),
...
('Super Admin', 'admin@ikooptic.co.ke', '{$passwordHash}', 'admin', datetime('now')),
('Content Manager', 'content@ikooptic.co.ke', '{$passwordHash}', 'content_manager', datetime('now')),
('Support Staff', 'support@ikooptic.co.ke', '{$passwordHash}', 'support', datetime('now'));
");
